    ...
    <section class="home">
        <div class="m-5">
            <table class="rTable">
                ...
                <tbody>
                    <?php
                    $result_cli = "SELECT * FROM users ORDER BY id ASC";
                    $result_cli = $pdo->prepare($result_cli);
                    $result_cli->execute();

                    while ($row_cli = $result_cli->fetch(PDO::FETCH_ASSOC)) {

                        echo "<tr>";
                        echo "<td>" . $row_cli['id'] . "</td>";
                        echo "<td>" . $row_cli['username'] . "</td>";
                        echo "<td>" . $row_cli['nivel'] . "</td>";
                        echo "<td>" . $row_cli['created_at'] . "</td>";
                        echo "<td> <a href='editcli.php?id=$row_cli[id]'>
                        <button>
                        <img src='img/botao-editar.png'>
                         </button>
                         </a> 
                         </td>";
                         echo "<td> <a href='apagarcli.php?id=$row_cli[id]'>
                        <button>
